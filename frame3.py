...
def car_loans():
	os.system('python3 car_loans.py')
	
def recommendations():
	os.system('python3 recommendations.py')
		...
		if family_members>max_seats:
			...
			if budget<min_price:
				...
				if count>0:
					...
					info_button = Button(frame3, text = 'MORE INFO', width = 9, fg='yellow', bg='#252525', font=('',14), bd=-2, command=more_info)
					info_button.place(x=500,y=450)
					info_text = Text(frame3, height=2, width=38, bg='#DCDCDC', font=('',13), bd=-2)
					info_text.insert(5.0, 'Note the CAR ID [given in square brackets].\nClick "MORE INFO" for Car Specifications.')
					info_text.config(state='disabled')
					info_text.place(x=500, y=500)
					
					recommendations_button = Button(frame3, text = 'OUR RECOMMENDATIONS', width = 19, fg='yellow', bg='#252525', font=('',14), bd=-2, command=recommendations)
					recommendations_button.place(x=680,y=450)
